...
namespace Error_NameNotFound.Model
{
    class Prozesstoken
    {
        ...
        public void Prozesscycle()
        {
            ...
            while (true)
            {
                var temp2 = LogicGates.gates_logic.FirstOrDefault(c => c.prozessnr == prozessnr);
                if (temp2 != null)
                {
                    for (int i = 0; i < LogicGates.connections.Count; i += 4)
                    {
                        ...
                        //if (temp!=null)
                        if (LogicGates.connections[i] == temp2.id)
                        {
                            var temp1 = LogicGates.gates_logic.FirstOrDefault(c => c.id == LogicGates.connections[i + 2]);
                            if (temp1.Input[LogicGates.connections[i + 3]] != temp2.Output[LogicGates.connections[i + 1]])
                            {
                                temp1.Inputset(temp2.Output[LogicGates.connections[i + 1]], LogicGates.connections[i + 3]);
